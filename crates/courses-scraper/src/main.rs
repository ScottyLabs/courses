...
fn main() -> Result<()> {
    ...
    let (stellic, term_joined) = Stellic::login(secrets.cookie_header.clone(), &args.export_root)?;
    ...
    let canvas = if needs_canvas {
        let token = secrets
            .canvas_token
            .clone()
            .context("CANVAS_TOKEN required for syllabi mode")?;
