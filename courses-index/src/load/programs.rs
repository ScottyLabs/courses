...
pub fn load_programs(programs_root: &Path, courses: &mut [Course]) -> Result<ProgramLoad> {
    ...
    let mut out = ProgramLoad::default();
    out.files_scanned = files.len();
    out.files_skipped = files.len() - parsed.len();
    ...
    match node {
        Value::Object(map) => {
            if map.get("type").and_then(|v| v.as_str()) == Some("course") {
                if let Some(course) = map.get("data").and_then(|d| d.get("course")) {
                    if let Some(code) = course.get("code").and_then(|v| v.as_str()) {
                        if !code.is_empty() {
