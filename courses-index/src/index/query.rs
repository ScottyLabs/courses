...
impl Searcher {
    ...
    pub fn query(&mut self, index: &Index, q: &Query) -> QueryResult {
        if self.cache_capacity > 0 {
            ...
            .is_ok()
            {
                if let Some(idx) = self.cache.iter().position(|e| e.key == self.cache_buffer) {
...
fn sort_by_f32(items: &mut Vec<(u32, f32)>, by_doc: &[f32], desc: bool) {
    items.sort_by(|a, b| {
...
fn sort_by_u32(items: &mut Vec<(u32, f32)>, by_doc: &[u32], desc: bool) {
    items.sort_by(|a, b| {
